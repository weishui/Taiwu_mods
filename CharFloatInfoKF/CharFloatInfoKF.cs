...
namespace CharFloatInfoKF
{
    ...
    internal static class ActorMenu_Awake_Patch
    {
        ...
        {
#if DEBUG
            Main.Logger.Log("CharFloatInfoKF.ActorMenu_Awake_Patch patched");
#endif
    ...
    internal static class WindowManage_WindowSwitch_Patch
    {
        ...
        private static void SetInfoMessage3(StringBuilder text, int actorId)
        {
            ...
            foreach (int i in DateFile.instance.baseSkillDate.Keys)
            {
                ...
                int typ2 = (i < 100 ? 501 : 500) + i;
                int skillValue = GetSkillGongFaValue(actorId, typ2);
                int skillDiffer = skillValue - int.Parse(DateFile.instance.GetActorDate(actorId, typ2, false));
                averageLevel += skillValue - skillDiffer;
                text.Append(i % 4 == (i < 100 ? 3 : 0) ? "\n" : "\t");
                if (i == 15)
                    text.Append("\n");
            }
            text.Append("　");
            text.Append(averageLevel / 30);
            text.Append("\n");
        }
